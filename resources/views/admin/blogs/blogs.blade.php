                    ...
                    <div class="row align-items-center ">
                        ...
                        <div class="col-md-4">
                            <div class="float-right d-none d-md-block">
                                <button onclick="resetFormFields()" type="button" id="add_blog_btn" data-toggle="modal" data-target="#BlogModal" data-dismiss="modal" aria-label="Close" class="btn btn-primary "><i class="fa fa-plus-circle"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
...
{{--                                                    <td>{!! $item->description !!}</td>--}}
                                                    ...
                                                    <td>
                                                        <a href="#" onclick="getBlogDetail({{$item->id}})" class="edit_blog_btn" data-toggle="tooltip" data-original-title="Edit">
                                                            <i class="m-2 fa fa-edit" aria-hidden="true"></i>
                                                        </a>
                                                        @if($item->status == 0)
                                                            <a href="{{ route('change-blog-status',['id'=>$item->id,'status'=>'1']) }}" msg="Are you sure to activate this blog?" class=" change_status" data-toggle="tooltip" data-original-title="Activate">
                                                                <i class="m-2 fa fa-thumbs-up"></i>
                                                            </a>
                                                        @elseif($item->status == 1)
                                                            <a href="{{ route('change-blog-status',['id'=>$item->id,'status'=>'0']) }}" msg="Are you sure to deactivate this blog?" class="  change_status" data-toggle="tooltip" data-original-title="Deactivate">
                                                                <i class="m-2 fa fa-thumbs-down"></i>
                                                            </a>
                                                        @endif
                                                        <a href="{{ route('change-blog-status',['id'=>$item->id,'status'=>'2']) }}" msg="Are you sure to delete this blog?" class=" change_status" data-toggle="tooltip" data-original-title="Delete">
